...
export class Habitacion extends BaseModel<HabitacionProps>{
    ...
    public static build(props: HabitacionProps){
        ...
        if(!(props.piso >= Habitacion.MIN_PISOS && props.piso <= Habitacion.MAX_PISOS)){
            throw new HabitacionError(`El numero de piso debe estar entre ${Habitacion.MAX_PISOS} y ${Habitacion.MAX_PISOS}`, 'piso');
        }

        if(!(props.numero >= Habitacion.MIN_NRO && props.numero <= Habitacion.MAX_NRO)){
            throw new HabitacionError(`El numero de la habitacion debe estar entre ${Habitacion.MIN_NRO} y ${Habitacion.MAX_NRO}`, 'numero');
        }

        if(!(props.cantCamas > Habitacion.MIN_CAMAS && props.cantCamas <= Habitacion.MAX_CAMAS)){
            throw new HabitacionError(`El numero de camas debe estar entre ${Habitacion.MIN_CAMAS} y ${Habitacion.MAX_CAMAS}`, 'cantCamas');
        }
